...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    # add resnet fuse
    if netG == 'resnet_9blocks_fuse':
        net = Resnet_fuse_Generator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)
...
class Resnet_fuse_Generator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        """
        # print('Resnet 9 Block Generator:')
        self.input_nc = input_nc
        assert(n_blocks >= 0)
        super(Resnet_fuse_Generator, self).__init__()
        self.opt = opt
        ...
        for i in range(n_downsampling):  # add downsampling layers
            ...
            if(no_antialias):
                ...
                encoder += [nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                          norm_layer(ngf * mult * 2),
                          nn.ReLU(True),
        ...
        for i in range(2):       # add ResNet blocks
            fuse_layer += [ResnetBlock(touch_channel + ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                decoder += [nn.ConvTranspose2d(touch_channel + ngf * mult, int(ngf * mult / 2),
                ...
                decoder += [Upsample(touch_channel + ngf * mult),
                          nn.Conv2d(touch_channel + ngf * mult, int(ngf * mult / 2),
        ...
        self.encoder_touch = nn.Sequential(*res)
        
        self.decoder = nn.Sequential(*decoder)

    def forward(self, input, layers=[], encode_only=False):
        img = input[:,:self.input_nc,:,:]
        touch = input[:,self.input_nc:,:,:]
        ...
        if len(layers) > 0:
            ...
            touch_feature = self.encoder_touch(touch)
            
            # concat
            up = nn.Upsample(size=(img_feature.shape[2], img_feature.shape[3]))
            touch_feature = up(touch_feature)
            combine_feature = torch.cat((img_feature, touch_feature), dim=1)

            fake = self.decoder(combine_feature)
            return fake
